...
export const findReservation = async (id: number): Promise<ReservationShape> => {
  return await Reservation.query()
    .select(
      ...
      raw(`jsonb_build_object('id', hs.id, 'name', hs."first_name" || ' ' || hs."second_name" || ' ' || hs."first_lastname" || ' ' || hs."second_lastname", 'email', us."email", 'photo', hs."url_picture") as host`),
      raw(`jsonb_build_object('id', ld.id, 'name', ld.name, 'type', tl.name, 'url_pictures', ld.url_pictures, 'comments', jsonb_build_object('qualification', ld.qualification, 'count', COUNT(distinct c))::jsonb) as lodging`),
      'reservations.start_date',
      'reservations.end_date',
      'reservations.night_value',
      'reservations.actual_state',
      'reservations.persons_amount'
    )
    .findById(id)
