...
export const createReservation = async (userId: number, lodgingId: number, reservation: ReservationShape) => {
  ...
  //if(lodging.user_id === userId) throw new UnauthorizedError("You are the owner of the lodging, you cannot make a reservation")

  if (!lodging.actual_state) throw new BusinessError("Lodging is deactivate");

  if (reservation.persons_amount > lodging.persons_amount) throw new BusinessError("The lodging does not support that many people");
...
export const listReservationsByUser = async (userId: number, code?: number, actual_state?: number, days?: number, page?: number): Promise<Page<Reservation>> => {
  const user = await UserRepository.findById(userId);
  ...
  return await ReservationRepository.listReservationsByUser(userId, code, actual_state, limit_date, page);
}

export const listReservationsByLodging = async (userId: number, lodgingId: number, code?: number, actual_state?: number, days?: number, page?: number): Promise<Page<Reservation>> => {
  const user = await UserRepository.findById(userId);
  ...
  return await ReservationRepository.listReservationsByLodging(lodgingId, code, actual_state, limit_date, page);
}
...
export const findReservationsForHost = async (userId: number, code?: number, actual_state?: number, days?: number, page?: number) => {
  const user = await UserRepository.findById(userId);
  ...
  return await ReservationRepository.findReservationsForHost(userId, code, actual_state, limit_date, page);
}